...
describe( 'crdt', () => {
	...
	describe( 'applyPostChangesToCRDTDoc', () => {
		...
		it( 'preserves clientIds when applying cached block-only changes', () => {
			applyPostChangesToCRDTDoc(
				doc,
				{
					blocks: [
			...
			const blocksArray = map.get( 'blocks' ) as YBlocks;
			...
			expect( updatedContent.toString() ).toBe( 'Updated' );
		} );

		it( 'updates clientIds when applying persistent block changes', () => {
			applyPostChangesToCRDTDoc(
				doc,
				{
					blocks: [
