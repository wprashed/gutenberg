...
describe( 'DimensionsPanel — per-control placeholder pattern', () => {
	...
	/*
	 * Regression: SpacingSizesControl flipped to custom-value mode for
	 * inherited padding/margin sides that resolved to numeric 0 (e.g.
	 * from a `var:preset|spacing|0` reference in Global Styles).
	 * `isValuePreset`/`isValueSpacingPreset` did not recognize numeric
	 * 0 as the `'0'` (None) preset, so PresetInputControl flipped into
	 * custom-value mode and rendered a UnitControl displaying `0`.
	 */
	it( 'treats numeric zero inherited padding sides as the None preset, not a custom value', () => {
		const settingsWithSpacingPresets = {
			...baseSettings,
			spacing: {
				...baseSettings.spacing,
		...
		// The padding control should remain in preset (slider) mode at the
		// None position rather than rendering custom UnitControl inputs.
		const paddingSliders = screen
			.getAllByRole( 'slider' )
			.filter( ( s ) =>
				/padding/i.test( s.getAttribute( 'aria-label' ) || '' )
			);
		expect( paddingSliders ).toHaveLength( 2 );
		expect( paddingSliders.every( ( s ) => s.value === '0' ) ).toBe( true );

		// No custom-value UnitControl should be rendered for padding.
		const paddingInputs = screen
			.queryAllByRole( 'spinbutton' )
			.filter( ( i ) =>
				/padding/i.test( i.getAttribute( 'aria-label' ) || '' )
			);
		expect( paddingInputs ).toHaveLength( 0 );
	} );
} );
